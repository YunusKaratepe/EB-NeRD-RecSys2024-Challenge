...
USE_COLUMNS = [
    "subtitle_tfidf_svd_sim",
    "subtitle_tfidf_svd_rn",
]
TARGET_COL = "body"
n_components = 50
...
def process_df(cfg, article_embeddings, articles_df, history_df, candidate_df):
    ...
    df = (
        candidate_df.with_columns(
            pl.Series(name="subtitle_tfidf_svd_sim", values=similarity)
        )
        .with_columns(
            pl.col("subtitle_tfidf_svd_sim")
            .rank(descending=True)
            .over("user_rn")
            .alias("subtitle_tfidf_svd_rn")
        )
        .select(USE_COLUMNS)
    )
