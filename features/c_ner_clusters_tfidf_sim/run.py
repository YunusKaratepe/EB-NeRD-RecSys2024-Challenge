...
USE_COLUMNS = [
    "ner_clusters_tfidf_sim",
    "ner_clusters_tfidf_sim_rn",
]


def process_df(cfg, articles_df, history_df, candidate_df):
    articles_df = articles_df.select(["article_id", "ner_clusters"]).with_columns(
        pl.col("ner_clusters").list.join(separator=" ")
    )
    # 集約してuserごとのner_clusters出現をテキスト化
    print("make user_df")
    user_df = (
        ...
        .group_by("user_id")
        .agg(pl.col("ner_clusters").cast(pl.String))
    )

    article_text_list = articles_df["ner_clusters"].cast(pl.String).to_list()
    ...
    df = (
        candidate_df.with_columns(
            pl.Series(name="ner_clusters_tfidf_sim", values=similarity)
        )
        .with_columns(
            pl.col("ner_clusters_tfidf_sim")
            .rank(descending=True)
            .over("user_rn")
            .alias("ner_clusters_tfidf_sim_rn")
        )
        .select(USE_COLUMNS)
    )
