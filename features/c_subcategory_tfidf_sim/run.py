...
USE_COLUMNS = [
    "subcategory_tfidf_sim",
    "subcategory_tfidf_sim_rn",
]


def process_df(cfg, articles_df, history_df, candidate_df):
    articles_df = articles_df.select(["article_id", "subcategory"]).with_columns(
        pl.col("subcategory").cast(pl.List(pl.Utf8)).list.join(separator=" ")
    )

    # 集約してuserごとのsubcategory出現をテキスト化
    user_df = (
        ...
        .group_by("user_id")
        .agg(pl.col("subcategory").cast(pl.String))
    )

    #
    article_text_list = articles_df["subcategory"].cast(pl.String).to_list()
    user_text_list = user_df["subcategory"].list.join(separator=" ").to_list()
    ...
    df = (
        candidate_df.with_columns(
            pl.Series(name="subcategory_tfidf_sim", values=similarity)
        )
        .with_columns(
            pl.col("subcategory_tfidf_sim")
            .rank(descending=True)
            .over("user_rn")
            .alias("subcategory_tfidf_sim_rn")
        )
        .select(USE_COLUMNS)
    )
