...
def create_feature(cfg: DictConfig, output_path):
    ...
    for data_name in ["train", "validation", "test"]:
        ...
        print(f"user_df shape: {user_df.shape}, columns: {user_df.columns}")
        user_df.write_parquet(
            output_path / "user_feat.parquet",
        )
        print(f"article_embedding_df shape: {article_embedding_df.shape}, columns: {article_embedding_df.columns}")
