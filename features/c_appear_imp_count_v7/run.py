    ...
    itertools.chain(
        *[
            [
                ...
                f"{group_name}_count_future_all",
                f"{group_name}_count_past_and_future_all",
            ]
            + [
            ...
            + [
                f"{group_name}_count_past_and_future_{time_str}"
                for time_str in USE_TIMES + FIX_WINDOWS
            ]
    ...
    for group_name, group_subset in [
        ...
        explode_df = explode_df.with_columns(
            [
                (
                    pl.col("const")
                    .rolling_sum_by("impression_time", window_size=time_str)
                    .over(group_subset)
                    - 1
        ...
        explode_df = explode_df.with_columns(
            [
                (
                    pl.col("const")
                    .rolling_sum_by("reverse_impression_time", window_size=time_str)
                    .over(group_subset)
                    - 1
        ...
        explode_df = explode_df.with_columns(
            [
                ...
                ).alias(f"{group_name}_count_past_and_future_all"),
            ]
            + [
                (
                    pl.col(f"{group_name}_count_past_{time_str}")
                    + pl.col(f"{group_name}_count_future_{time_str}")
                ).alias(f"{group_name}_count_past_and_future_{time_str}")
                for time_str in USE_TIMES + FIX_WINDOWS
            ]
        )
